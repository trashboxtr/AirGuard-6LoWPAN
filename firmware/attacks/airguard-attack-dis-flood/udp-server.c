...
static uint32_t attack_udp_rx_count;
...
print_root_metric(void)
{
  ...
  LOG_INFO("AIRGUARD_METRIC,role=server,node=%u,time_ms=%lu,reachable=%d,"
           ...
           "attack_mode=%u,attack_udp_rx=%" PRIu32 ","
           ...
           (unsigned)AIRGUARD_ATTACK_MODE, attack_udp_rx_count,
...
/*---------------------------------------------------------------------------*/
static void
attack_udp_rx_callback(struct simple_udp_connection *c,
                       const uip_ipaddr_t *sender_addr,
                       uint16_t sender_port,
                       const uip_ipaddr_t *receiver_addr,
                       uint16_t receiver_port,
...
PROCESS_THREAD(udp_server_process, ev, data)
{
  ...
  simple_udp_register(&attack_udp_conn, ATTACK_UDP_SERVER_PORT, NULL,
                      ATTACK_UDP_CLIENT_PORT, attack_udp_rx_callback);

  LOG_INFO("AIRGUARD_BOOT,role=server,node=%u,time_ms=%lu,"
           "metric_interval_s=%lu,attack_mode=%u\n",
           node_id, simulation_time_ms(),
           (unsigned long)(METRIC_INTERVAL / CLOCK_SECOND),
           (unsigned)AIRGUARD_ATTACK_MODE);
